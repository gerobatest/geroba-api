...
let modeExistant = require('../models/modeExistant.model'); //pour exiger le model 
...
router.route('/').get((req, res) =>{
    modeExistant.find() // obtenir les donnée depuis la base de données
...
router.route('/add').post((req, res) => { //une requete Post 
    ...
    const newModeExistant = new modeExistant ({
        para1, 
        para2
    });

    newModeExistant.save()
     .then(() => res.json('Content (modeExistant ) added!'))
