...
let modeDepot = require('../models/modeStocks.model'); //pour exiger le model 
...
router.route('/').get((req, res) =>{
    modeDepot.find() // obtenir les donnée depuis la base de données
...
router.route('/add').post((req, res) => { //une requete Post 
    ...
    const newModeDepot = new modeDepot ({
        para1, 
        para2
    });

    newModeDepot.save()
     .then(() => res.json('Content (modeStocks) added!'))
