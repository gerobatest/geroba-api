...
let tarification = require('../models/tarification.model'); //pour exiger le model 
...
router.route('/').get((req, res) =>{
    tarification.find() // obtenir les donnée depuis la base de données
...
router.route('/add').post((req, res) => { //une requete Post 
    ...
    const newTarification = new tarification ({
        para1, 
        para2
    });

    newTarification.save()
     .then(() => res.json('Content (modeTravaux) added!'))
